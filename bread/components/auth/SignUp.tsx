...
export function SignUp({ onSwitchToSignIn, onSuccess }: SignUpProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        toast.success("Account created successfully! Welcome to the baking community!");
        onSuccess();
      }
    } catch (error) {
      ...
      if (error instanceof Error) {
        if (error.message.includes("fetch")) {
          toast.error("Could not connect to the registration service. Please try again later.");
        } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            id="username"
            type="text"
            placeholder="BakerExtraordinaire"
            value={username}
