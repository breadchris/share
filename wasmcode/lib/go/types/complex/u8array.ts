import {AbstractTypeSpec, ReadResult} from "../spec";
import {Uint8} from "../basic";
...
export class UInt8ArrayTypeSpec extends AbstractTypeSpec<Uint8Array> {
  ...
  constructor(length: number) {
    super(`[${length}]${Uint8.name}`, Uint8.size * length, Uint8.alignment, 0)
  ...
  write(view: DataView, addr: number, val: Uint8Array, buff: ArrayBufferLike) {
    if (!(val instanceof Uint8Array)) {
      throw new Error(`${this.constructor.name}.write: value should be Uint8Array (got: ${val.constructor.name})`)
    }
