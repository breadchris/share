	...
	return Nav(Class("bg-gray-800 p-4 text-white"),
		Div(Class("container mx-auto flex justify-between items-center"),
			...
			Div(Class("space-x-4"),
				...
				A(Href("/zine-creator"), Class("hover:underline"), T("Zine")),
			),
		),
	)
}
...
func Index() string {
	bodyContent := Div(Class("container mx-auto text-center mt-16"),
		...
		Div(Class("flex justify-center space-x-4"),
			...
			A(Href("/zine-creator"), Class("bg-emerald-500 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded"), T("Zine")),
		),
	)
