...
export default function AIEditorDemo({ id: propsId, onSave, initialContent }: AIEditorDemoProps) {
  ...
  useEffect(() => {
    ...
    // Use props id if provided, otherwise check URL, otherwise generate new
    if (propsId) {
      setDocId(propsId);
    } else if (currentId) {
      setDocId(currentId);
    } else {
      // Generate new ID and reload page with it
  ...
  return (
      <YDocProvider docId={docId} authEndpoint={async (): Promise<any> => {
