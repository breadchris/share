...
func (cf *ClaudeFollowupExecutor) Spawn(ctx context.Context, taskID uuid.UUID, worktreePath string) (*exec.Cmd, error) {
	fmt.Printf("Debug: ClaudeFollowupExecutor.Spawn called with taskID: %s, worktreePath: %s, sessionID: %s\n", taskID.String(), worktreePath, cf.SessionID)
	// Validate worktree path exists
	if _, err := os.Stat(worktreePath); os.IsNotExist(err) {
		slog.Debug("Worktree path does not exist, creating it", "path", worktreePath)
		if err = os.MkdirAll(worktreePath, 0755); err != nil {
			return nil, fmt.Errorf("failed to create worktree path: %s, error: %w", worktreePath, err)
		}
	}
